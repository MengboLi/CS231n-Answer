...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    bn_param['running_var'] = running_var
     # 这里的意思是，对于输入的N个数据，求所有数据在每个维度的均值，然后每个数据点在它的每个维度上减去均值，除以方差
    return out, cache
...
def batchnorm_backward(dout, cache):
    ...
    d_sample_mean = np.sum(dx_ * (-1.0/np.sqrt(sample_var + eps)) ,axis=0) + d_sample_var * (np.sum(-2*(x - sample_mean),axis=0)/N)
    dx = dx_ * (1.0/np.sqrt(sample_var+eps)) + d_sample_var * 2*(x - sample_mean)/N + d_sample_mean * 1.0/N
#     print(d_sample_var.shape)
#     print((2*(x - sample_mean)/N).shape)
...
#     dx_ = gamma* dout
#     dsample_var = np.sum(-1.0/2*dx_*x_/(sample_var+eps), axis =0)
#     dsample_mean = np.sum(-1/np.sqrt(sample_var+eps)* dx_, axis = 0) # drop the second term which simplfies to zero
#     dx = 1/np.sqrt(sample_var+eps)*dx_ + dsample_var*2.0/N*(x-sample_mean) + 1.0/N*dsample_mean
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    feature_vector_mean = np.mean(x,axis = 1)
    feature_vector_mean = np.vstack(feature_vector_mean)
    feature_vector_var = np.sum((x - np.vstack(feature_vector_mean))**2,axis=1) / D
    feature_vector_var = np.vstack(feature_vector_var)
    
    x_ = (x - feature_vector_mean) / np.sqrt(feature_vector_var + eps)

    out = gamma * x_ + beta
    cache = (x, eps, gamma, beta, x_, feature_vector_mean, feature_vector_var)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x, eps, gamma, beta, x_, feature_vector_mean, feature_vector_var = cache
    N,D = x.shape
    ...
    d_feature_vector_var = np.vstack(np.sum(dx_*(x-feature_vector_mean),axis=1)) *(-0.5)* (feature_vector_var+eps)**(-3.0/2.0)

    d_feature_vector_mean = np.vstack(np.sum(dx_*(-1.0/np.sqrt(feature_vector_var+eps)),axis=1)) + d_feature_vector_var * np.vstack(np.sum(-2*(x-feature_vector_mean),axis=1)/D)

    dx = dx_*(1.0/np.sqrt(feature_vector_var+eps)) + d_feature_vector_var * 2*(x-feature_vector_mean)/D + d_feature_vector_mean*1.0/D
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # 对于组归一化来说，所有的步骤其实和批归一化都是相同的，只是将C各个通道分为G各组，每个组分别利用BN进行归一化，
    # 归一化结束后再用reshape把维度转换为,N, C, H, W
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
