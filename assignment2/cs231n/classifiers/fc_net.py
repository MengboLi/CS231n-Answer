    ...
    def loss(self, X, y=None):
        ...
        for layer in range(self.num_layers):
            ...
            if self.normalization == 'layernorm' and layer!= self.num_layers-1:
                gammai, betai = self.params['gamma%d' % (layer + 1)], self.params['beta%d' % (layer + 1)]
                outi, ln_cachei = layernorm_forward(outi, gammai, betai, self.ln_params[layer])
                ln_cache_list.append(ln_cachei)
        ...
        for layer in list(range(self.num_layers,0,-1)):
            ...
            if self.normalization == 'layernorm' and layer!= self.num_layers:
                dout, dgamma, dbeta = layernorm_backward(dout, ln_cache_list[layer-1])
                grads['gamma%d' % (layer)] = dgamma
                grads['beta%d' % (layer)] = dbeta
            #backforward
            ...
            dout = dxi
            #　等价于下面的这句话，但是在逻辑上下面的这句话是很不好的，虽然意思是一样的，不利于建立整体的反向传播逻辑。
